...
return view.extend({
	...
	render: function() {
		...
		function startUpgrade(isForce, keepConfig) {
			...
			var keepText = keepConfig ? '保留系统配置' : '不保留系统配置（仅保留本插件）';
			var msg = isForce
		...
		return E('div', {'class': 'cbi-map'}, [
			...
			E('div', {'class': 'cbi-section', style: 'margin-bottom:16px;padding:20px;'}, [
				...
				E('div', {style: 'font-size:12px;color:#b45309;background:#fffbeb;border:1px solid #fde68a;border-radius:4px;padding:8px 10px;'}, "提示：在点击'检查更新'前，请先在'Release 地址'栏内填入固件在GitHub的 tag 链接，接着依次点击'解析'与'保存配置'完成配置。"),
